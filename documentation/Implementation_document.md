...
### Performance testing
The class for performance testing is used to record the time that each algorithm cost in graphs with different size. Using this separate class to do the performance testing can avoid wasting time to wait for the rendering of graphs.
...
function Astar():
  ...
  Labyrinths generated by randomized DFS and randomized Prim have different features. Labyrinths generated by randomized DFS have a low branching factor and contain many long corridors. Labyrinths generated by randomized Prim are biased towards many short dead ends and their paths are usually shorter than paths in labyrinths generated by randomized DFS. This feature affects the performance of path finding algorithm. Also different heiristic functions have an effect on the performance of A*. For more details please check the [testing documentation](https://github.com/yumoL/walkingAMaze/blob/master/documentation/Testing_document.md). 
  
  ## Improvements
  - I compared BFS and A* in this project, other advanced shortest path finding algorithms such as LPA* and HPA* could be also compared if there was time left.
  - The HashMap that I implemented can only save Node as a key and Integer as a value, it would be better if the HashMap can save different types of data using generic.
  - I used MinHeap to implement the PriorityQueue, Fibonacci heap could be used to improve the efficiency. 
  - The entrance and exit of a labyrinth are defaulted to be in the outmost of the labyrinth. It would be interesting to check a labyrinth where entrance or exit is in the inner labyrinth. 
