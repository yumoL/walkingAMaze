...
- The performance of A* implemented using Manhattan distance is better than A* implemented using Euclidean distance. 
  * Reason: Manhattan distance = |x1-x2|+|y1-y2| and Euclidean distance = sqrt[(x1-x2)^2+(y1-y2)^2], which means Manhattan distance is longer than Euclidean distance and therefore closer to the real distance between nodes. Traversing less nodes make A* implemented using Manhattan distance become more efficiency than A* implemented using Euclidean distance. 
  
##### All path finding algorithms
- A* implemented using squared Euclidean distance finds a path most quickly. 
  * Reason: Distance between two nodes estimated by squared Euclidean distance is always longer than the real distance, it traversed substantially less nodes than other algorithms (see the third and forth graph) and therefore can find a path faster than other algorithms. On the other hand, A* implemented using squared Manhattan distance isn't guaranteed to find the shortest path. 

- A* implemented using squared Euclidean distance works more stably in graphs generated by randomized Prim than graphs generated by randomized DFS (see the fifth graph). 
