...
### Performance Testing
In performance tests,  we would like to know how long it takes an algorithm to find the path without needing to see the concrete path, so I used a (seperate class)[https://github.com/yumoL/walkingAMaze/blob/master/src/main/java/performanceComparison/Comparator.java] to do the performance testing to save time, because when the size of the graph increases, it takes quite much time to wait for the rendering of the graph. 
